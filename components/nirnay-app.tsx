...
import { getEnrichedProjects, getMonitoringProjects, getCompletedProjects, getDashboardProjects, getAttentionProjects, getRiskStatistics, getMLWarnings, getMLPredictionByProjectId, getProjectHistoryData, getPortfolioHistoryData, filterProjects, getUniqueMinistries, getUniqueSectors, getUniqueStatuses, isMonitoringEligible } from '@/lib/ml-predictions'
...
function FilterBar({onFilter, allRows, rows, defaultStatus='All Statuses'}:{onFilter:(m:string,s:string,r:string,st:string)=>void, allRows:Project[], rows:Project[], defaultStatus?:string}) {
  const [m,setM]=useState('All Ministries');
  const [s,setS]=useState('All Sectors');
  const [r,setR]=useState('All Risk Levels');
  const [st,setSt]=useState(defaultStatus);
  ...
  return <div className="filterbar"><div className="filter-label"><Filter size={15}/> Filters</div><select value={m} onChange={e=>change(e.target.value,s,r,st)} aria-label="Ministry"><option>All Ministries</option>{mList.map(x=><option key={x}>{x}</option>)}</select><select value={s} onChange={e=>change(m,e.target.value,r,st)} aria-label="Sector"><option>All Sectors</option>{sList.map(x=><option key={x}>{x}</option>)}</select><select value={r} onChange={e=>change(m,s,e.target.value,st)} aria-label="Risk level"><option>All Risk Levels</option>{['Critical','High','Medium','Low'].map(x=><option key={x}>{x}</option>)}</select><select value={st} onChange={e=>change(m,s,r,e.target.value)} aria-label="Project lifecycle"><option>All Statuses</option>{stList.map(x=><option key={x}>{x}</option>)}</select><button className="button ghost" onClick={()=>change('All Ministries','All Sectors','All Risk Levels',defaultStatus)}>Reset</button><ExportButton rows={rows}/></div> 
}
function Kpis({rows, counts, label='Active Portfolio'}:{rows:Project[], counts: Record<string, number>, label?:string}){const critical=rows.filter(p=>p.riskLevel==='Critical').length,high=rows.filter(p=>p.riskLevel==='High').length,medium=rows.filter(p=>p.riskLevel==='Medium').length,low=rows.filter(p=>p.riskLevel==='Low').length;const exposure=formatCr(rows.reduce((a,p)=>a+Math.max(0,p.revisedCost-p.originalCost),0));const avgDelay = rows.length ? Math.round(rows.reduce((a,p)=>a+p.delayRisk,0)/rows.length) : 0;return <div className="kpi-grid">{[['Total Projects',rows.length||0,label],['Critical Risk',critical,'Requires immediate review'],['High Risk',high,'Priority monitoring'],['Avg Delay Risk',avgDelay+'%','Portfolio average'],['Cost Risk Exposure',exposure,'Model estimate'],['Early Warnings',critical,'Active signals']].map(([label,value,meta],i)=><div className="kpi" key={String(label)}><div className="kpi-top"><span>{label}</span><span className={`kpi-icon k${i}`} /></div><strong>{value}</strong><small>{meta}</small></div>)}</div>}
function ProjectTable({rows,title='Projects Requiring Attention'}:{rows:Project[];title?:string}){return <Card title={title} subtitle="Projects with the highest predicted implementation risk"><div className="table-wrap"><table><caption className="sr-only">{title}</caption><thead><tr><th>Project</th><th>Ministry</th><th>Sector</th><th>Risk</th><th>Cost Risk</th><th>Delay Risk</th><th>Primary Driver</th><th>Reporting Period</th><th>Action</th></tr></thead><tbody>{rows.length?rows.map(p=><tr key={p.id}><td><Link className="project-link" href={`/projects/${p.id}`}>{p.name}</Link><small>{p.id}</small></td><td>{p.ministry}</td><td>{p.sector}</td><td><RiskBadge level={p.riskLevel} score={p.riskScore}/></td><td>{p.costRisk}%</td><td>{p.delayRisk}%</td><td>{p.primaryDriver}</td><td>{p.reportDate || 'Mar 2025'}</td><td><Link className="view-link" href={`/projects/${p.id}`}>View <ChevronRight size={14}/></Link></td></tr>):<tr><td colSpan={9}><div className="empty">No projects found<br/><button className="text-button">Clear filters</button></div></td></tr>}</tbody></table></div></Card>}
...
function Dashboard(){
  ...
  useEffect(() => {
    // Dashboard = active monitoring portfolio only (excludes Completed)
    getMonitoringProjects().then(r => { setAllRows(r); setRows(r); });
  }, []);
  ...
  // Early warnings: only monitoring-eligible projects (already filtered above)
  ...
  return <><HomeHero/><PageHeader title="Infrastructure Project Monitoring" subtitle="Active monitoring portfolio — excludes completed projects"/><Notice/><FilterBar allRows={allRows} rows={rows} onFilter={handleFilter}/><Kpis rows={rows} counts={counts} label="Active Portfolio"/><ProjectTable rows={getAttentionProjects(rows, 5)}/><div className="two-col"><Distribution counts={counts}/><Trend/></div><Card title="Early Warning Signals" subtitle="ML signals requiring administrative review — active projects only"><div className="warnings">{warnings.length?warnings.map(w=><div className="warning-row" key={w.id}><div className="warning-icon"><Zap size={17}/></div><div><b>{w.title}</b><strong>{w.project}</strong><p>{w.detail}</p><small>{w.metric}</small></div><Link href={`/projects/${w.id}`} className="button secondary">View Project</Link></div>):<div className="empty">No warnings for current selection</div>}</div></Card></>
}
...
function ProjectsPage(){
  ...
  const rows = filterProjects(allRows, query, 'All Ministries', 'All Sectors', risk, status);
  const stList = getUniqueStatuses(allRows);
  
  return <><PageHeader title="All Projects" subtitle="Explore and review the national infrastructure project portfolio"/><Notice/><div className="explorer-tools"><div className="searchbox"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search projects..." aria-label="Search projects"/></div><select value={risk} onChange={e=>setRisk(e.target.value)} aria-label="Risk filter"><option>All Risk Levels</option><option>Critical</option><option>High</option><option>Medium</option><option>Low</option></select><select value={status} onChange={e=>setStatus(e.target.value)} aria-label="Lifecycle filter"><option>All Statuses</option>{stList.map(x=><option key={x}>{x}</option>)}</select><button className="button secondary"><Filter size={15}/> More filters</button><ExportButton rows={rows}/></div><Card title="Project Portfolio" subtitle={`${rows.length} records in current view`}><div className="table-wrap"><table><thead><tr><th>Project</th><th>Ministry</th><th>Sector</th><th>Original Cost</th><th>Expenditure</th><th>Physical Progress</th><th>Risk</th><th>Status</th></tr></thead><tbody>{rows.length?rows.map(p=><tr key={p.id}><td><Link className="project-link" href={`/projects/${p.id}`}>{p.name}</Link><small>{p.id}</small></td><td>{p.ministry}</td><td>{p.sector}</td><td>{formatCr(p.originalCost)}</td><td>{formatCr(p.expenditure)}</td><td>{p.physicalProgressRaw === null ? 'Not reported' : `${Number(p.physicalProgressRaw).toFixed(1)}%`}</td><td>{p.lifecycle === 'Completed' ? <span className="badge">COMPLETED</span> : <RiskBadge level={p.riskLevel} score={p.riskScore}/>}</td><td><span className="status-badge">{p.lifecycle||p.status}</span></td></tr>):<tr><td colSpan={8}><div className="empty">No projects found for current filter</div></td></tr>}</tbody></table></div></Card></>
}
...
function Detail({id}:{id:string}){
  ...
  const isCompleted = p.lifecycle === 'Completed';
  const physProg = p.physicalProgressRaw === null ? 'Not reported' : `${Number(p.physicalProgressRaw).toFixed(1)}%`;
  const finProg = p.financialProgress == null || Number.isNaN(p.financialProgress) ? 'Not available' : `${Number(p.financialProgress).toFixed(1)}%`;
  
  return <><PageHeader title={p.name} subtitle={`${p.id} · ${p.ministry} · ${p.sector} · ${p.state}`}/><div className="detail-hero"><div><span className="eyebrow">{isCompleted ? 'COMPLETED PROJECT RECORD' : 'PROJECT RECORD'}</span><h2>{p.name}</h2><p>Implementing agency: {p.implementingAgency}</p></div>{isCompleted ? <span className="status-badge" style={{fontSize:'16px', padding:'10px'}}>COMPLETED</span> : <RiskBadge level={p.riskLevel} score={p.riskScore}/>}</div><div className="tabs">{['Overview','Financial','Schedule','Milestones','Risk Analysis','Interventions','History'].map(t=><button key={t} className={tab===t?'selected':''} onClick={()=>setTab(t)}>{t}</button>)}</div><div className="detail-grid">{[['Approved cost',formatCr(p.originalCost)],['Revised cost',formatCr(p.revisedCost)],['Expenditure',formatCr(p.expenditure)],['Start date','Not available'],['Planned completion',p.plannedCompletion||'Not available'],['Expected completion',p.expectedCompletion||'Not available'],['Physical progress',physProg],['Financial progress',finProg],['Implementing agency',p.implementingAgency||'Not available'],['State / region',p.state||'Not available'],['Project status',p.lifecycle||p.status],['Assessment',isCompleted?'Historical Outcome':'Model-based estimate']].map(([l,v])=><div className="detail-item" key={l}><small>{l}</small><b>{v}</b></div>)}</div><div className="two-col detail-sections">
  <Card title="ML RISK ASSESSMENT" subtitle={isCompleted ? "Historical model assessment — project completed." : "Model-based predictive analysis"}>
    {isCompleted && (
      <div className="risk-metrics" style={{marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid var(--border)'}}>
        <div><b style={{fontSize:'1.8rem', color:'var(--text)'}}>{p.timeOverrunMonths||0}</b><span>Months Delayed (Final)</span></div>
        <div><b>{formatCr(p.revisedCost-p.originalCost)}</b><span>Cost Overrun (Final)</span></div>
      </div>
    )}
...
function GenericPage({title,subtitle}:{title:string;subtitle:string}){
  const [allRows, setAllRows] = useState<Project[]>([]);
  const [rows, setRows] = useState<Project[]>([]);
  useEffect(() => { getMonitoringProjects().then(r => { setAllRows(r); setRows(r); }); }, []);
  ...
  if (title === 'Model Performance') return <><PageHeader title={title} subtitle={subtitle}/><Notice/><FilterBar allRows={allRows} rows={rows} onFilter={handleFilter}/><Card title="Model Metrics" subtitle="Validated Model: Random Forest (scikit-learn)"><div className="table-wrap"><table><thead><tr><th>Metric</th><th>Value</th><th>Interpretation</th></tr></thead><tbody><tr><td>Selected Model</td><td>Random Forest Classifier</td><td>Top performer across Macro F1 & Brier score</td></tr><tr><td>Test Accuracy</td><td>95.6%</td><td>Chronological temporal test evaluation</td></tr><tr><td>Balanced Accuracy</td><td>95.8%</td><td>Balances delayed and non-delayed performance</td></tr><tr><td>Macro F1 Score</td><td>95.4%</td><td>Unweighted mean of class F1 scores</td></tr><tr><td>ROC-AUC / PR-AUC</td><td>0.979 / 0.990</td><td>Discriminative ranking power</td></tr><tr><td>Brier Score (Calibration)</td><td>0.0379</td><td>Calibrated probability certainty</td></tr><tr><td>Class 0 (Not Delayed) F1</td><td>94.4%</td><td>Precision: 92.3%, Recall: 96.7%</td></tr><tr><td>Class 1 (Delayed) F1</td><td>96.4%</td><td>Precision: 97.9%, Recall: 95.0%</td></tr></tbody></table></div></Card></>;
